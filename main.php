...
echo $timestamp;
echo @strColor('nevy',"\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
echo "JANGAN LUPA BERDOA DAN BERSEDEKAH";
echo @strColor('nevy',"\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
echo "\n";

echo @strColor('green',"Pilih :\n");
echo @strColor('nevy', " 1. Register \n 2. Login\n");
echo @strColor('green', "Pilihan (1/2) ? ");
$pilihbarang = trim(fgets(STDIN));
echo "\n";
if($pilihbarang == 1) {  // masuk menu register
    echo "\n";
    echo @strColor("green","REGISTER\n");
    echo @strColor("green","===================================\n");
    $sesid = sessionId();
    $uniqid = uniqueId();
    $nama = name();
    $email = strtolower(str_replace(" ", "", $nama) . mt_rand(100,999999) . "@gmail.com");
    echo @strColor("green","No. HP : ");
    ...
    //echo "\n";
        echo @strColor("red",$errormsg);
        echo "\n\n";
        echo @strColor("green","Ulangi ? ( y / n ) : ");
        $ulangi = trim(fgets(STDIN));
        if ($ulangi == "y") {
          goto awal;}
        else {
          echo "\n";
          echo @strColor("nevy", "Exiting script .....");
          echo "\n\n\n";
          die;}
    }
    ...
    print_r($asdzzz1);
    echo "\n";
    echo @strColor("green","OTP PIN : ");
    $otpsetpin = trim(fgets(STDIN));
    $data = '{"pin":"'.$pin.'"}';
    $verify = postpin("https://customer.gopayapi.com/v1/users/pin", $data, $otpsetpin, $sesid, $uniqid, $toknew, $id);
    print_r($verify);
    echo "\n".@strColor("yellow","PIN OK")."\n";
    echo "\n".@strColor("green"," PIN ANDA $pin ")."\n";
    echo @strColor("green","  PIN ANDA $pin ")."\n";
    //echo @strColor("red"," SEMOGA CUAN ")."\n";
    ...
    aloman:
/*
echo @strColor("red","Notice !!");
echo @strColor("green","Lakukan Topup  Terlebih Dahulu : \n");
echo @strColor("blue"," - Untuk BCA Pake Kode 70001+nope (ex 70001081245xxxx) \n");
echo @strColor("green"," - Untuk BRI Pake Kode 301341+nope (ex 301341081245xxxx) \n");
echo @strColor("nevy"," - Admin Fee Rp 1.000 Min. Top Up Rp 10.000\n\n");
*/

    echo @strColor("green","Sudah Top UP ? ( y / n ) : ");
    ...
    for($r1; $r1 < $r2; $r1++){
      ...
      echo @strColor("green","$r1+1,. Saldo : ");
      while($duitmu = 0) {
        sleep(2);
        $duitmu=@$duit["data"][0]["balance"]["value"];
      }
      echo @strColor("green",rupiah($duitmu));
      
      $landing = getRequest("https://api.gojekapi.com/goclub/v1/landing-page", $uniqid, $sesid, $toknew, $id);
      $expnya = json_decode($landing, true);
      $member_xp = @$expnya["data"]["current_tier"]["member_xp"];

      echo @strColor("green"," EXP : ",$member_xp);
      //echo "\n";
      if ($member_xp > 3900){
          echo @strColor('red', "Anda telah mencapai Limit EXP!");
          die;
      }
      /*if($duitmu == "0"){
            echo "\n";
            echo @strColor("green","Tunggu Jon, Duitmu Belum Balik..");
        sleep(10);
        }*/
      echo "\n";
//ini utk scan 500k
...
//echo "\n";
...
} else if ($pilihbarang == 2) {  // masuk menu login
    echo "\n";
    autologin:
    echo @strColor("green","LOGIN\n");
    echo @strColor("green","===================================\n");
    echo @strColor("green","No. HP : ");
    $nohp = trim(fgets(STDIN));
    $sesid = sessionId();
    ...
    echo @strColor("green","Nama   : $namany\n");
    echo @strColor("green","PIN    : ");
    //echo @strColor("yellow"," Masukkan PIN Anda : ");
    $pin = trim(fgets(STDIN));
    $data = '{"pin":"'.$pin.'"}';
    ulangpinotpd:
    $pinnya = postsend("https://customer.gopayapi.com/v1/users/pin", $data, $sesid, $uniqid, $toknew, $id);
    ...
    if ($errormsg1 != "PIN sudah terpasang") {
        ...
        //print_r($verify);
        /*
        echo "\n".@strColor("yellow","  PIN SUDAH TERPASANG ")."\n";
        echo "\n".@strColor("green"," PIN ANDA $pin ")."\n";
        echo @strColor("green","  PIN ANDA $pin ")."\n";
        echo @strColor("red"," SEMOGA CUAN ")."\n";
        */
    ...
    echo @strColor('green', "SALDO = ");
    echo @strColor('green', rupiah($duitmu));
    //sleep(5);
    echo @strColor("green", " -- EXP = $member_xp");
    echo "\n\n";
    echo @strColor("green","Rolling Scan (35x@Rp.500.000) ? ( y/n ) : ");
    //echo "\n";
    $yakah = trim(fgets(STDIN));
    $r1= "0";
    $r2= "7";
    ...
    while($member_xp <= $limitscan) {
        ...
        echo rupiah($test99);*/
        echo "\n";
        echo @strColor('green', $r1++);
        echo @strColor('green',". SCAN Status = ");
        ...
        if ($test99 == true) {
            echo @strColor('blue', "Success! Delay 25s.");
            $member_xp += 100;
            sleep(25);
        } else {
            echo @strColor('red', "Failed! Delay 30s.");
            sleep(30);
        }
    ...
    if ($member_xp > $limitscan) {
        echo @strColor('red', "\nAnda telah mencapai limit EXP!\n\n");
        goto redemkupon;
    }
}

redemkupon:
echo @strColor("green", "Redeem Voucher CB Alfamart : \n");
echo "\n";
...
//echo @strColor("nevy","Tunggu Sejenak..\n");
if($namasatu == "Cashback GoPay 10rb jajan & belanja"){
    echo @strColor("green",$namasatu);
    $idb = $satu;
    echo " ";
    ...
    echo @strColor("green",$statusclaim);
    echo "\n";
    goto ambilterusssc;
}elseif($namadua == "Cashback GoPay 10rb jajan & belanja"){
    echo @strColor("green",$namadua);
    echo " ";
    $idb = $dua;
    $data = '{"select_benefit":"'.$idb.'"}';
    $payment = postpatchklem("https://api.gojekapi.com/goclub/v1/user-claims/".$tc, $data, $uniqid, $sesid, $toknew, $id);
    ...
    //echo "\n";
    //echo @strColor("purple","Ulang Lagi Ya");
    echo "\n";
    goto ulanglagiyaac;
}
//}
...
echo @strColor("yellow","My Voucher : \n");
echo " 1 . $q\n";
echo " 2 . $w\n";
echo " 3 . $e\n";
echo " 4 . $r\n";
...
function strColor($color = "default" , $text) {
    $arrayColor = array(
        'grey'      => '1;30',
        'red'       => '1;31',
        'green'     => '1;32',
